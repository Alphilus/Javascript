...
class DemoApplicationTests {
	...
	void save_favorites() {
		List<User> users = userRepository.findByRole(UserRole.USER);
		List<Movie> movies = movieRepository.findMovieByStatus(true);
	...
	void save_histories() {
		...
		for (User user : users) {
			...
			for (Movie movie : historyMovies) {
				if(movie.getType() == MovieType.PHIM_BO) {
					List<Episode> episodes = episodeRepository.findByMovie_IdOrderByDisplayOrder(movie.getId());
					Episode episode = episodes.get(random.nextInt(episodes.size()));
					History history = History.builder()
							.duration((double) random.nextInt(episode.getDuration()))
							.createdAt(LocalDateTime.now())
							.user(user)
							.movie(movie)
							.episode(episode)
					...
					History history = History.builder()
							.duration((double) random.nextInt(movie.getType() == MovieType.PHIM_LE ? 120 : 45)) // Assuming 120 min for movies and 45 min for other types
							.createdAt(LocalDateTime.now())
							.user(user)
							.movie(movie)
							.build();
	...
	void test_movie_query() {
		...
		// Sort
		List<Movie> moviesSort = movieRepository.findByType(MovieType.PHIM_BO, Sort.by("name", "rating").descending());
	}
